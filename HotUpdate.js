...
import ProgressBar from './CusProgressBar';
...
class HotUpdate extends Component {
    ...
    syncImmediate() {
        if(!this.state.next){
            CodePush.checkForUpdate('2SjAc9C0gujXdZjoyOuZypcN6b9k4ksvOXqog').then((update) => {
                console.log(update);
                if (update) {
                ...
                <View style={styles.container}>
                    <View style={[{width: 0.8 * SWidth, marginBottom: 5}]}>
                        ...
                        <View style={{backgroundColor: '#fff', width: 0.8 * SWidth, borderBottomLeftRadius: 5, borderBottomRightRadius: 5, alignItems: 'center'}}>
                            <Text style={{color: '#2979FF', fontSize: 20, fontWeight: 'bold', justifyContent: 'center', marginTop:5}}>发现新版本</Text>
